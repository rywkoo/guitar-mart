...
class User(db.Model):
    ...
    user_id = db.Column(db.String(10), unique=True, nullable=False)  # <-- new public ID
    ...
    def generate_user_id():
        """Generate public user ID like U00001, U00002..."""
        last_user = User.query.order_by(User.id.desc()).first()
        next_id = 1 if not last_user else last_user.id + 1
        return f"U{next_id:05d}"
...
class RegisterToken(db.Model):
    ...
    @staticmethod
    def generate(email):
        token = secrets.token_hex(4)  # 8-character hex
