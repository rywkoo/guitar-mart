...
# =============================
# GET ADMIN FROM COOKIE (HTTP-only)
# =============================
def get_current_admin():
    """Check if user is admin using HTTP-only cookie."""
    ...
    try:
        ...
        print(f"[ADMIN JWT ERROR] {e}")
        return None


# -----------------------------
# Admin Dashboard Pages (HTML) – Use Cookie
# -----------------------------
@admin_bp.route("/")
def admin_index():
...
def get_user(user_id):
    """Return JSON for a single user (used by the modal)."""
    current_user = User.query.get(get_jwt_identity())
    if not current_user or current_user.role != "admin":
        return jsonify({"error": "Forbidden"}), 403
