...
def is_admin():
    """Check if current user is admin (optional JWT)."""
    try:
        # This will NOT raise error if no JWT
...
# =============================
# Admin Pages (HTML) - Optional JWT
# =============================
@admin_bp.route("/")
@jwt_required(optional=True)  # Allows access without JWT
def admin_index():
    if not is_admin():
...
def add_product():
    ...
    db.session.commit()
    return jsonify({"success": True, "product": {"id": product.id}}), 201
...
def edit_product(product_id):
    ...
    db.session.commit()
    return jsonify({"success": True, "product": {"id": product.id}}), 200
