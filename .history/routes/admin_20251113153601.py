...
def purchase_report():
    ...
    if report_type == "daily":
        ...
        invoices = query.filter(Invoice.created_at >= start_day,
                                Invoice.created_at < end_day).all()
        total = sum(inv.total_amount for inv in invoices)
        labels = [start_day.strftime("%Y-%m-%d")]
        totals = [total]
        ...
        for i in range(6, -1, -1):  # last 7 days
            ...
            end_day = start_day + timedelta(days=1)
            day_total = sum(inv.total_amount for inv in query.filter(
                Invoice.created_at >= start_day,
                Invoice.created_at < end_day
            ).all())
            labels.append(start_day.strftime("%Y-%m-%d"))
        ...
        try:
            ...
            invoices = query.filter(Invoice.created_at >= start_dt,
                                    Invoice.created_at <= end_dt).all()
