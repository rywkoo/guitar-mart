...
def purchase_report():
    ...
    if report_type == "daily":
        ...
        query = query.filter(Invoice.created_at >= start_of_week)
    elif report_type == "monthly":
        query = query.filter(
            func.strftime("%Y-%m", Invoice.created_at) == now.strftime("%Y-%m")
        )
    ...
    # Aggregate totals per invoice or overall
    total_amount = sum(inv.total_amount for inv in invoices)
    total_invoices = len(invoices)

    # Return list of invoices as well
    invoice_list = [
        {
            "id": inv.id,
