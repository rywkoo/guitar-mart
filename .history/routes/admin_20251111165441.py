...
# @rywkoo | November 11, 2025 | Cambodia (KH)
...
def get_user_from_token():
    """Extract JWT from cookies or headers and return user if valid."""
    token = None
    # 1. Try cookies (from set_access_cookies)
    ...
    if auth.startswith("Bearer "):
        token = auth.split(" ")[1]
    # 3. Try localStorage fallback (not sent, but we'll use JS)
    ...
    try:
        ...
        user_id = data["sub"]  # Flask-JWT-Extended uses 'sub'
...
# =============================
# Admin Pages (HTML) - NO @jwt_required
# =============================
@admin_bp.route("/")
def admin_index():
    if not is_admin():
...
def add_product():
    ...
    product = Product(name=name, price=float(price), stock=int(stock), category_id=int(category_id), image=filename)
...
def edit_product(product_id):
    ...
    if category_id: product.category_id = int(category_id)
    if image and allowed_file(image.filename):
        if product.image:
            old = os.path.join(UPLOAD_FOLDER, product.image)
