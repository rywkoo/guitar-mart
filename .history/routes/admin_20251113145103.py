...
@jwt_required()  # Keep JWT if needed, otherwise remove for pure Postman testing
def edit_product(product_id):
    ...
    # Skip image for now to avoid multipart issues
...
# -----------------------------
# PUBLIC: GET single product (POSTMAN READY)
# -----------------------------
@admin_bp.route("/api/products/<int:product_id>/public", methods=["GET"])
def get_product_public(product_id):
    """
    Return a single product as JSON for testing in Postman.
    No JWT required.
    """
    product = Product.query.get_or_404(product_id)
    return jsonify({
